...
- `the relevant base résumé` — CV for proof points in drafts
...
2. Read `the relevant base résumé` for proof points
...
2. **Sentence 2:** One concrete value-add from the report's Block B match or a proof point from the relevant base résumé. Quantify if possible.
