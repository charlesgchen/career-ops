...
- `cv.tex` — CV for proof points in drafts
...
2. Read `cv.tex` for proof points
...
2. **Sentence 2:** One concrete value-add from the report's Block B match or a proof point from cv.tex. Quantify if possible.
