...
4. **CV** at `your base résumés` + `article-digest.md` — read for proof points
...
- **Background red flags** — gaps, transitions, unusual elements from `your base résumés` + `_profile.md`. Honest, specific, forward-looking framing — never defensive.
...
For each gap, suggest: "You need a story about {topic}. Consider: {specific experience from your base résumés that could become a STAR+R story}."
...
- **What to lead with**: stack-relevant proof points from `your base résumés` / `article-digest.md`.
